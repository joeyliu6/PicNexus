...
describe('filterMonthPoints', () => {
  ...
  it('容器高度为 0 返回空数组', () => {
    const periods = [{ year: 2024, month: 0, count: 10 }];
    expect(filterMonthPoints(periods, new Set(), 0)).toEqual([]);
  });

  it('单个月份返回单个点', () => {
    const periods = [{ year: 2024, month: 5, count: 10 }];
  ...
  it('边界点始终保留', () => {
    const periods = [
      { year: 2024, month: 11, count: 100 },
      { year: 2024, month: 6, count: 1 },  // 低密度但不是边界
      { year: 2024, month: 0, count: 100 },
    ];
    const result = filterMonthPoints(periods, new Set(), 800);
  ...
  it('已加载月份标记为 isLoaded', () => {
    const periods = [{ year: 2024, month: 3, count: 10 }];
  ...
  it('多月份结果按位置排序', () => {
    const periods = [
      { year: 2024, month: 6, count: 50 },
      { year: 2024, month: 3, count: 50 },
      { year: 2024, month: 0, count: 50 },
    ];
    const result = filterMonthPoints(periods, new Set(), 800);
  ...
  it('count 属性正确传递', () => {
    const periods = [{ year: 2024, month: 3, count: 42 }];
    const result = filterMonthPoints(periods, new Set(), 800);
