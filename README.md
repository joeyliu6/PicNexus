...
Obsidian 插件随 PicNexus Release 提供 `picnexus-obsidian-*.zip`，也可以通过 BRAT 从独立插件仓库安装。解压安装时，将 `main.js`、`manifest.json` 和 `styles.css` 放入当前仓库的 `.obsidian/plugins/picnexus/`，然后在 Obsidian 第三方插件设置中启用 PicNexus。

插件需要 PicNexus 桌面端保持运行，并在「设置 → 外部编辑器」中启用 Obsidian 集成、选择图床。默认端口为 `36799`。详细说明见 [插件 README](plugins/picnexus/README.md)。
