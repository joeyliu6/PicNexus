...
- [x] **取消后续检不丢结果**（2.2，2026-08-21 过）
      ...
      > 秒结束根本来不及点取消。第一版方案「传 15 张到 dufs 再停掉 dufs」当时也不行：局域网 HTTP
...
- [x] **3xx 不再误判失效**（2.3，2026-08-20 过）
      ...
      **不是**「失效」，也没写成「防盗链限制」。
      ...
      > 复现脚本见 `scripts/.acceptance-2026-08-20.local/redirect-server.js`（当时必须绑回环：
      > 旧版 `validate_probe_url` 把局域网 HTTP 判成「策略拦截」。该缺陷 2026-08-21 已修——
      > 探测判据对齐了上传/渲染/CSP 三处，局域网字面量与已确认主机名放行，脚本绑局域网 IP
      > 也可以了。经过见 [lightbox-gate-and-upyun-dual-chain-2026-08-20.md](./audits/lightbox-gate-and-upyun-dual-chain-2026-08-20.md) 的「链接检测拦掉所有局域网图床」小节）。
