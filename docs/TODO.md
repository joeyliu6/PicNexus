...
### [ ] config + mirror 扫描剩余两条发现（P1-2/P1-3）——待真机复测后再修
...
- **症状**：P1-2 mirror 文档承诺切主/删镜像走「脏标记同步」，机制不存在，内容改动进不了云端；
  P1-3 灯箱单条重检不重算 `linkCheckSummary`
- **定性**：两条均已单元级复现，源码在审计附录；均为 🟡
...
额外改动）→ 真机复验通过，详见审计文档「执行记录」区（改动尚未提交，待用户确认后拆两次提交）。
