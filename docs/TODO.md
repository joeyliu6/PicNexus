...
**还剩 Q1（未修，比预估严重）**：`UpyunUploader` 把「操作员账号/密码」当 S3 的 AK/SK 用。
实测坐实——同一个桶，操作员凭证 403 `ErrInvalidAccessKeyID`，控制台生成的 S3 专用凭证 200。
**这意味着 GUI 上传对每个照着界面标签填写的用户都从未成功过**，不是配置麻烦而是功能全坏。

⚠️ 设置页「测试连接」走 REST、GUI 上传走 S3，**测试连接亮绿灯不能证明上传可用**——这是它潜伏至今的原因。

修法要改配置结构（加 `s3AccessKey`/`s3SecretKey`）+ 设置页两个输入框，
六处改动已列在审查文档的「剩下的活：Q1」，其中一项（必填门槛怎么卡）待用户拍板。
凭证已存 `scripts/.upyun.local`，回归直接跑脚本即可，不必再找用户要账号。
