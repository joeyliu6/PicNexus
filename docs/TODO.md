...
### [ ] 又拍云三个缺陷已修，待真机 GUI 验收

- **来源**：2026-08-19，issue #4（S3 Content-Type）收尾时发现又拍云是漏网的一条，顺查发现漏的不止它
- **状态**：D1（P0 丢数据）/ D2 / Q1 全部已修，协议层已真机验证；**只剩设置页 + 主界面的人工点击验收**
- **详情与判据**：[docs/audits/upyun-audit-2026-08-19.md](./audits/upyun-audit-2026-08-19.md)

根因一句话：五个私有存储里，只有又拍云走自家 REST API 而非 S3 SDK，
于是公共流程 `build_upload_key()`（贴 path 前缀 + 文件名唯一化）它一样都没吃到。
**判断后续改动是否又漏了它，看审查文档里那张对比表。**

**已修（2026-08-19，真机验证 + 回归哨兵）**：

- ~~**D1（P0）**：编辑器/CLI 链路用原始文件名，同名图片第二次上传无声覆盖第一次。~~
  实测坐实（67 字节 → 126 字节，回读只剩第二张），已改用 `build_upload_key`。
- ~~**D2（P2）**：编辑器链路不认设置页的「存储路径」。~~ 已补齐；实际是**四**层缺 `path`，
  审查初稿漏了「前端变更指纹」那层——不补它的话，用户单独改存储路径不会下发到后台。
- ~~**Q2**：SigV4 的 region 写死 `'upyun'`。~~ 实测排除，又拍云 S3 端点不校验 region，不用改。

- ~~**Q1**：`UpyunUploader` 把「操作员账号/密码」当 S3 的 AK/SK 用。~~ 实测坐实——同一个桶，
  操作员凭证 403 `ErrInvalidAccessKeyID`，控制台生成的 S3 专用凭证 200。**GUI 上传对每个
  照界面标签填写的用户都从未成功过**，不是配置麻烦而是功能全坏。已加 `s3AccessKey`/`s3SecretKey`
  两个字段并改用它们签名。

⚠️ 设置页「测试连接」走 REST、GUI 上传走 S3，**测试连接亮绿灯不能证明上传可用**——这是它潜伏至今的原因。

又拍云是唯一一个**两条链路用两套凭证**的图床，必填门槛因此按链路分开卡：GUI 上传要 S3 凭证，
编辑器/CLI/测试连接只要 operator/password（否则只用 Typora、手头没 S3 凭证的老用户会被无谓地卡住）。

**升级影响**：老用户的又拍云在主界面上传里会显示「未配置」，直到补填 S3 凭证；
Typora / Obsidian / CLI 不受影响。

**只剩真机 GUI 验收**：设置页填两套凭证 → 主界面拖图上传 → 确认成功。这步只能人工点。
凭证已存 `scripts/.upyun.local`，协议回归直接跑脚本即可，不必再找用户要账号。
