...
### [ ] config + mirror 扫描四条发现（P0：托盘旧钥匙回写毁配置）——待真机复测后再修
...
- **症状**：P0-1 换备份密码后常驻托盘 webview 仍持旧钥匙，托盘一勾选图床/切主题就用旧钥匙整份覆盖
  `.settings.dat`，重启被 `main.ts` 重置且无备份（「设置密码」方向不可恢复）；P1-1 `configStore.get`
  递出缓存原件，`saveSettings` 就地改写后保存失败的回滚空转；P1-2 mirror 文档承诺切主/删镜像走「脏标记同步」，
  机制不存在，内容改动进不了云端；P1-3 灯箱单条重检不重算 `linkCheckSummary`
- **定性**：四条均已单元级复现（8 条用例全命中，源码在审计附录）；P0-1 🔴，其余 🟡
- **优先级**：高 —— P0-1 丢整份配置

**当前处置**：**先真机复测、再动代码**（用户 2026-09-07 要求）。复测步骤与结果表在审计文档「真机复测清单」，
P0-1 复测会真的清掉配置，做之前先导出备份。修复顺序 P0-1 → P1-1 → P1-2（先改文档）→ P1-3。
