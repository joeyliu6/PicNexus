...
- [ ] **就地改一个字符能存住**（`7e61952` 的核心功能，旧判据里没有）
      点进密码框 → 只改末尾一个字符 → 点别处失焦（芯片跳「✓ 已更新」约 2 秒）→
      **托盘退出后重启** → 用改过的密码测试连接。
      **判据**：连接成功。这条同时验证「聚焦即取值」真的把原值取了出来——
      没取出来的话你改出来的是个只有一个字符的密码，必然连不上。
- [ ] **看一眼不等于改一次**（核心不变量的人工验证，**两条路径都要走**）
