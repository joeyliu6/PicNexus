...
## 补记（2026-08-20）：当时补的只是缩略图，还漏了三条

上面这段修的是收藏页 / 时间线的**缩略图**。发版前复核发现同一批地址还有三条路没过闸：
灯箱大图与模糊占位、表格悬停预览、灯箱 ±1 预热。灯箱由三个视图共用，
所以收藏页 / 时间线**点开大图照样绕过**。已于 2026-08-20 统一收进 `useGatedImageUrls`，
详见 [lightbox-gate-and-upyun-dual-chain-2026-08-20.md](./lightbox-gate-and-upyun-dual-chain-2026-08-20.md)。

教训：撤掉一道**全局**兜底（CSP）之后，要枚举的是"所有加载图片的路径"，
而不是"最近改过的那几个组件"。
