...
| 组 | 内容 | 结果 |
|----|------|------|
| G1 | 图床链路 Digest 四形态 | 4/4 |
| G2 | 备份链路 Digest 四形态（本轮新增能力） | 4/4 |
| G3 | 成功路径（文案透传 + 207 未被误伤） | 2/2 |
| G4 | 换备份密码后的跨进程存活（[TODO.md](../TODO.md) 挂了很久的两条） | 2/2 |

过程中挖出 **1 个既有缺陷**（连接测试全线显示 `[object Object]`，已修）。
...
[TODO.md](../TODO.md) 里挂了很久的那两条，本轮清空。**这是唯一三套测试框架都表达不了的部分。**

#### G4-1 · 换钥 → 托盘退出重启 → 配置同步

| 步骤 | 客观证据（不是「看起来成功了」） |
|------|--------------------------------|
| 设备份密码 | `cli-config.json` SHA256 `F4775ACF…` → `52B0CA59…`，仍是 `PNXCLI1:` 加密态；`.settings.dat` `C2830A10…` → `5BB02A27…` |
| 换钥编排顺序 | 日志三行为序：`密钥已更新到系统钥匙串` → `cli-config.json 已用新密钥重新加密` → `字段级密文搬运完成：共发现 6 处，6 条已换新钥匙`（0 孤儿） |
| **托盘退出后重启** | 进程完全退出（非隐藏到托盘）后重启，日志 `从钥匙串读取现有密钥` → `[Store] ✓ 解密成功` ×4 → `[useConfig] ✓ 配置加载成功`，**无 `BackupPasswordRequiredError`，且不需要用户重新输密码** |
| 备份 WebDAV 测试连接 | `验证成功 / WebDAV 连接正常`，状态「已连接」——证明搬运过的 WebDAV 密码本身能用 |
| 上传配置到云端 | 服务端 `picnexus/settings.json` 13171 字节，开头 `PNXPWD:/RPyewb9p28jS4G…` |
| 明文残留扫描 | `password` / `secret` / `token` / `cookie` / `webdav` / `admin123` / `accessKey` **一个都搜不到** |

最后两行一次钉住两件事：密文密码解得开（解不开根本传不了），且整份配置上传前确实被加密。
**这也是删掉 `useWebDAVSync` 的实证**——它那条上传路径没有这道加密，接上线就是把上面
七个关键词对应的真实凭据裸推到服务器。

#### G4-2 · 换钥后用图床 WebDAV 传图

| 判据 | 结果 |
|------|------|
| 落盘文件 | `pics/g4-test_20260817_tfc4.png` |
| 唯一后缀 | ✅ 原名 + 日期 + 4 位随机，防同名覆盖生效 |
| 逐字节比对 | ✅ 1555 字节，SHA256 与源文件一致 |
| **匿名** GET 公开链接 | ✅ HTTP 200 + `Content-Type: image/png` + 内容与源文件一致 |

最后一条不带任何认证——这才是 Markdown 里那张图能显示的真正前提。
...
```
[TrayMenuWindow] 读取配置失败，沿用上一次快照:
{"name":"BackupPasswordRequiredError","message":"需要输入备份密码才能解密配置"}
```

托盘菜单是独立 webview，它自己的 `SecureStorage` 实例还停在换钥前的模式，读到 `PNXPWD:`
前缀就抛错。**不是缺陷**：有降级兜底（`TrayMenuWindow.vue:185` 沿用上一次快照，不崩），
重启后 0 次、完全自愈，且那段代码不是本轮引入。记为观察，不阻塞发版。
...
- **vitest**：`invoke` 全部被 mock，钥匙串根本不存在
- **mocked E2E**：`set_secure_key` 是 no-op、`get_or_create_secure_key` 返回硬编码常量，
  换过钥的密文在那里必然解不开
- **wdio 真机 E2E**：一个 session 只能启动一次应用，且 `withGlobalTauri: false`
  导致脚本无法预置状态

测试③ 已经把「换钥后新起的 `SecureStorage` 实例能解开搬运结果」覆盖到了，
但那仍是同一个进程内的模拟。**真正的系统钥匙串跨进程往返只有真机能证**，本轮已证。

## 遗留

无。本轮四组判据全部通过，[TODO.md](../TODO.md) 中挂起的两条真机验收已清空。
