...
提示会**自己区分**这三种情况，照着念即可（Digest 那句限图床三条链路 + 备份的「测试连接」，
唯一没有的是同步过程中的 PUT/GET，见下方 ⚠️；另外两句所有链路都有）：

| 提示 | 含义 | 怎么办 |
|------|------|--------|
| 「服务端要求 Digest 认证，PicNexus 暂不支持」 | 密码没错，是认证方式不匹配 | 在服务端启用 Basic（多数 NAS 可切换），或换用支持 Basic 的服务 |
| 「认证失败，请检查用户名和密码」 | 401：服务端提供了 Basic，凭证确实对不上 | 真的去核对密码 |
| 「访问被拒绝，请检查账号对该路径的读写权限」 | 403：账号认过了，是这个路径不让它读写 | 别动密码。去服务端把该账号对这个目录的读写权限打开，常见于只读共享、目录 ACL 没配、按路径/IP 做了限制 |
...
- 403 **不读**这个头，也**不共用 401 的文案**——那是「认过了但没权限」，不带方案协商，
  说成认证失败会把用户支去改本来就没错的密码。它单独走 `WEBDAV_FORBIDDEN_MESSAGE`。
