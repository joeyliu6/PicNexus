...
|------|--------|---------|---------|
| 没存过 | 空，显示原始占位文案 | 无 | 禁用 |
| 存过了、没聚焦 | **32 个圆点**（多行 512 个，自然折行铺满） | 「✓ 已保存」芯片 | 可用 |
...
框内那行灰字不再常驻。它只在你**把光标放进去**、真的可能要改的那一刻才出现。

### 清空即删除（2026-08-20）

**把框清空再离开 = 删除这一项**，不是"留空则不修改"。清除前会弹一次确认。

> ⚠️ 这里也有过一次反复，记下来免得下一个人再绕。
>
> 原本框里写的是「留空则不修改」，而且**已保存的凭证根本删不掉**——内置图床不像
> WebDAV 能整个删掉 profile，只能覆盖。用户想清空又拍云的 S3 凭证时卡在这里。
>
> 这套文案跟本组件的交互模型是矛盾的：**框一聚焦就会把真值填进来**，所以想"留空"
> 必须主动删——而主动删就是删的意思。旧文案等于在用户刚做完删除动作时告诉他
> 「你这下不算」。
>
> 曾经想过**另加一个「清除」按钮**把两件事分开。否掉了：用户的原话是
> 「不删了就是删了，还要额外加清除按钮？」——他是对的，见下面那笔账。

判断「用户是不是真要删」全靠 `useSensitiveDraft` 里 `originals` 这笔账：
它**只在真值确实填进框里之后**才被记上。于是「框是空的」这四种来路一刀切分开：

| 来路 | 记账了吗 | 结果 |
|------|---------|------|
| 真值填进来过、用户删光了 | 有 | **弹确认 → 删** |
| 解密失败，值没进过框 | 没有 | 不动 |
| 解密还在路上人就走了（一路 Tab 会大量制造） | 没有 | 不动 |
| 压根没聚焦过 | 没有 | 不动 |

**别把这个判断简化成"空就删"或"空就不删"**：前者会让解密抖一下就抹掉好凭证，
后者就是退回改动前"删不掉"的状态。`useSensitiveDraft.spec.ts` 有 7 条钉着这四种来路，
三种变异（一律删 / 一律不删 / 跳过确认）都做过阴性对照。

**为什么要确认框**：「全选删掉准备重填」和「就是不要了」在事件上完全一样。前者中途
被打断就会丢一把钥匙，而很多服务商的 SecretKey 只在生成时显示一次，丢了得重新生成，
会连带搞挂别处在用它的程序。ESC / 点叉叉一律当**不删**——判断不出来的时候，
宁可留着一把用户想删的钥匙。

**考虑过但否掉**：直接删 + 给个「撤销」提示条。撤销要求在内存里攥着明文好几秒，
跟本契约「明文只在输入框里活一瞬」直接冲突。

确认框上有个「**本次运行内不再提示**」勾选框，勾了之后本次运行不再弹，**重启恢复**。

> ⚠️ **不要改成永久记住（落配置）。** 这个框防的是「删到一半被打断」这种意外，
> 而会去点「不再提示」的人一定是在**故意删**的那一刻点的——拿一次故意操作换掉
> 以后所有意外的保护，代价不对等。真正会让人烦的是「一口气连删好几个」
> （重配图床、跑验收），那本来就是一次会话内的事，记到会话结束正好够用。
>
> 两条实现细节，改的时候别踩掉：
>
> - **只在点「清除」时才认这个勾。** 勾了却点取消 / 按 ESC，那次交互的结论是
>   「不要删」，顺手关掉保护不是用户的意思。
> - **静音状态必须是模块级的**，不能按 composable 实例存。四个设置分组各调一次
>   `useSecretClearConfirm()`，按实例存就会「在 S3 卡片点了不再提示、切到 Token
>   卡片又弹」——用户眼里那就是没生效。
>
> `useSecretClearConfirm.spec.ts` 8 条钉着这些，两种变异（取消也生效 / 按实例存）
> 都做过阴性对照。

⚠️ **加密类字段的 `commit` 遇到空串必须直接写空**，不能 `encrypt('')`——
那会存进一段非空密文，`hasStored` 继续为真，界面上就是「删了但还显示已保存」。
目前 WebDAV 图床密码和 WebDAV 备份密码两处都各自处理了。
...
> 📌 **这条只对「没聚焦」那一态成立。** 聚焦即取值之后，框里是真值被原生遮蔽的圆点，
> 个数当然等于真实长度。想知道长度得先主动点进这个框——不再是"扫一眼界面就看见"。
> 这是就地编辑必然要付的代价，接受它，别为此回退到空框。

**别给圆点加 `letter-spacing`。** 曾经有过 `0.12em`，只作用在 `::placeholder` 上；
聚焦后那排圆点由 `type="password"` 原生遮蔽渲染，拿不到这个字距——于是点进去的
一瞬圆点会"挤"一下，看着像内容变了。两边用的都是 U+2022、同一套字体，**不设字距
就自动对齐**，字距是这里唯一能造成差异的东西。
...
多行用 512 个圆点、**不带换行**，交给浏览器自然折行。曾经拼的是「3 行 × 32 个」，
三行一样长，一看就是生成的。自然折行后宽度跟着框走，框宽变了也不用重算。

placeholder 不是内容、不驱动滚动条，超出可视区的部分直接裁掉，不会冒出滚动条。

### 个数取「铺满并有富余」，不追求精确

单行 32、多行 512，都远超一屏能显示的量，多出来的部分被裁掉。

不去算"刚好铺满"是因为算不准：框宽随窗口宽度和 grid 列数变，多行还带
`resize: vertical`（用户能拖高）。任何精确值都会在某个宽度上露出空白，
而空白正是本设计要消灭的那个信号。裁掉的代价是零，露白的代价是又一轮返工。

> ⚠️ 多行铺满依赖调用方在 `input-class` 里给 `word-break: break-all`
> （见 `CookieServiceGroup` 的 `.cookie-field`）。`•` 的断行属性是 AL，
> 整串圆点在浏览器眼里是一个不可断的长单词——没有 break-all 就不折行，
> 512 个会摊成横向溢出的一行。新增 multiline 调用点时记得带上。
