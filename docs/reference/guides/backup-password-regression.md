...
| 做法 | 说明 |
|------|------|
| 便携版（最干净） | exe 同级建 `data\portable.json`，数据全走 `data\`，与正式版彻底隔离；**也是唯一能「一键还原」的做法** |
| 备份后用 dev | 先把 `.settings.dat` 复制一份到别处，测完还原<br>🚨 **只对「不设备份密码」的第一轮成立**——一旦设过备份密码，钥匙串里的旧密钥被覆盖且无备份，那份文件就**永久解不开**了，还原它会毁掉配置。详见[测完收尾](#测完收尾) |
| 直接用 dev | 只在你不在意当前配置时 |

> ⚠️ **别把上面第二行当成安全网。** 「备份了就能还原」这个直觉在密钥轮转面前不成立：
> 备份的是**密文**，而解开它的钥匙已经被换掉了。要真正可回滚，只有便携版那条路。
...
> 📌 **顺序限制已经解除。** 这里曾经有一条「先设备份密码、再填 WebDAV 密码，
> 顺序不能反」的约束——因为设备份密码会换钥匙、把已存的 WebDAV 密码变成孤儿。
> 该缺陷已于 2026-08-15 修复（换钥匙时内层密文一并搬运），两种顺序现在都安全。
> 如果**仍然**看到「密码解密失败，请重新输入密码后再测试」，那就是回归了，
> 对照 [orphan-field-ciphertext.md](../troubleshooting/orphan-field-ciphertext.md) 排查。
...
>
> 跑过第二轮之后的正确收尾：**保留当前 `.settings.dat`**，备份文件直接删掉。
> 注意这条和上面坑 3 是两回事——孤儿密文缺陷已经修了，**当前配置里的密码都是好的**，
> 不需要重填；作废的只有那份用旧密钥加密的**备份文件**。
>
