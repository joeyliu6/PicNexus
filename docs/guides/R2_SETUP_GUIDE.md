...
> 本指南适用于 WeiboDR-Uploader v3.0 多图床架构

Cloudflare R2 是 v3.0 支持的 5 个图床之一，适合长期稳定存储。本指南将帮助您配置 Cloudflare R2 备份功能。

**提示**：R2 是可选图床，如果您只需要临时存储，可以使用 TCL 或京东图床（开箱即用，无需配置）。详见 [TCL/京东使用指南](TCL_JD_GUIDE.md)。
