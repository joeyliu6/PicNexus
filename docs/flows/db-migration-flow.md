...
PicNexus 的 SQLite 数据库由 `@tauri-apps/plugin-sql` 驱动,所有 schema 管理集中在 `src/services/database/HistoryDatabase.ts`。
...
> **关键源文件**:`src/services/database/HistoryDatabase.ts`(L41 路径、L72 open、L82~L121 CREATE TABLE、L140~L144 迁移调用、L243~L318 三个迁移函数)、`src/App.vue`(L23 导入、L60~L74 健康检查)
...
flowchart TD
    ...
    F1 --> F2[CREATE TABLE IF NOT EXISTS<br/>history_items 18 字段]
    F2 --> F3[CREATE INDEX IF NOT EXISTS<br/>× 5 条索引]
    F3 --> F4[CREATE TABLE IF NOT EXISTS<br/>sync_log 6 字段]

    F4 --> G[顺序执行迁移]
    G --> G1[migrateAddFavoriteColumn]
    G1 --> G2[migrateAddSuccessCountColumn]
    G2 --> G3[migrateAddSuccessfulServiceIdsColumn]
...
展示单个迁移函数的内部逻辑。以 `migrateAddSuccessCountColumn` 为例(L264~L290)。

> **关键源文件**:`src/services/database/HistoryDatabase.ts` L166~L169 `columnExists`、L264~L290 样例迁移
...
| 迁移函数 | 添加列 | 回填策略 | 行号 |
|----------|--------|----------|------|
| `migrateAddFavoriteColumn` | `is_favorited BOOLEAN DEFAULT 0` | 无需回填 | L243~L258 |
| `migrateAddSuccessCountColumn` | `success_count INTEGER DEFAULT 0` | `json_each(results)` 统计成功数 | L264~L290 |
| `migrateAddSuccessfulServiceIdsColumn` | `successful_service_ids TEXT` | `json_group_array` 从 results 提取 service_id | L296~L318 |
...
```mermaid
flowchart TD
    A[需求:给 history_items 加新字段] --> B[1. 在 HistoryDatabase.ts 新增迁移函数<br/>migrateAddXxxColumn]
    ...
    F --> G[5. 同步修改 types.ts 的<br/>HistoryItem 接口类型]
    G --> H[6. 同步修改 fromRow/toRow<br/>序列化映射]
    H --> I{查询语句要改吗?}
