...
压缩命令会在解码前解析图片头并执行 5000 万像素预检；图片头无法解析时直接拒绝。WebP 输出额外限制单边不超过 16383 像素，编码器错误会作为可读错误返回，不会触发内部 panic。

各上传命令会在读取文件内容前检查 metadata 大小。已有服务限制保持不变（5–200MB）；牛客、纳米、七鱼、知乎和 S3 兼容上传统一使用 50MB 上限，避免超限或稀疏文件先触发大块内存分配。
