...
fn detect_image_kind(bytes: &[u8]) -> Option<DetectedImageKind> {
    if bytes.len() >= 3 && bytes[0] == 0xFF && bytes[1] == 0xD8 && bytes[2] == 0xFF {
        return Some(DetectedImageKind::Jpeg);
    }
    if bytes.len() >= 8 && bytes.starts_with(&[0x89, b'P', b'N', b'G', 0x0D, 0x0A, 0x1A, 0x0A]) {
        return Some(DetectedImageKind::Png);
    }
    if bytes.len() >= 6 && (bytes.starts_with(b"GIF87a") || bytes.starts_with(b"GIF89a")) {
        return Some(DetectedImageKind::Gif);
    }
    if bytes.len() >= 12 && bytes.starts_with(b"RIFF") && &bytes[8..12] == b"WEBP" {
        return Some(DetectedImageKind::Webp);
    }
    if bytes.len() >= 2 && bytes.starts_with(b"BM") {
        return Some(DetectedImageKind::Bmp);
    }
    if bytes.len() >= 4 && (bytes.starts_with(b"II*\0") || bytes.starts_with(b"MM\0*")) {
        return Some(DetectedImageKind::Tiff);
    }
    if bytes.len() >= 4 && bytes.starts_with(&[0x00, 0x00, 0x01, 0x00]) {
        return Some(DetectedImageKind::Ico);
    }
    if bytes.len() >= 12 && &bytes[4..8] == b"ftyp" {
        ...
        {
            return Some(DetectedImageKind::Avif);
        }
    }
    ...
    if lower.starts_with("<svg") || (lower.starts_with("<?xml") && lower.contains("<svg")) {
        return Some(DetectedImageKind::Svg);
    }
...
fn validate_svg_dimensions(bytes: &[u8]) -> Result<(), String> {
    let svg = std::str::from_utf8(bytes).map_err(|_| "SVG 内容不是有效 UTF-8".to_string())?;
    ...
    if let Some(view_box) = extract_svg_attr(svg, "viewBox") {
        let parts = view_box
            .split(|c: char| c.is_ascii_whitespace() || c == ',')
            .filter(|part| !part.is_empty())
            .collect::<Vec<_>>();
        if parts.len() == 4
            && parse_svg_number(parts[2]).is_some()
            && parse_svg_number(parts[3]).is_some()
        {
            return Ok(());
        }
    }
...
fn validate_image_bytes(
...
) -> Result<DetectedImageKind, String> {
    if bytes.is_empty() {
        return Err("请求体为空，请发送图片二进制数据".to_string());
    }
    if bytes.len() > MAX_SERVER_UPLOAD_SIZE {
        return Err(format!(
            "图片过大 ({:.1}MB)，本地上传接口最大支持 {}MB",
    ...
    if let Some(ct) = normalized_content_type.as_deref() {
        if !ct.starts_with("image/") && ct != "application/octet-stream" {
            return Err(format!("Content-Type 不是图片: {}", ct));
        }
    }
    ...
    if let Some(ct) = normalized_content_type.as_deref() {
        if ct.starts_with("image/") && !kind.matches_content_type(ct) {
            return Err(format!(
                "Content-Type 与图片内容不匹配：{} 不是 {}",
                ct,
                kind.display_name()
            ));
        }
    ...
    validate_detected_image_payload(bytes, kind)?;
...
fn validate_image_file(path: &std::path::Path) -> Result<DetectedImageKind, String> {
    ...
    if metadata.len() as usize > MAX_SERVER_UPLOAD_SIZE {
        return Err(format!(
            "图片过大 ({:.1}MB)，本地上传接口最大支持 {}MB",
...
fn auth_value_matches(value: &str, expected: &str) -> bool {
    !expected.is_empty() && value.trim() == expected
}

fn request_has_valid_server_token(
    headers: &HeaderMap,
    query: Option<&str>,
    expected: &str,
) -> bool {
    ...
    if headers
        .get(SERVER_AUTH_TOKEN_HEADER)
        .and_then(|value| value.to_str().ok())
        .is_some_and(|value| auth_value_matches(value, expected))
    {
        return true;
    }
    ...
    query.is_some_and(|query| {
        url::form_urlencoded::parse(query.as_bytes()).any(|(key, value)| {
            matches!(key.as_ref(), "token" | "authToken" | "auth_token")
                && auth_value_matches(value.as_ref(), expected)
        })
    })
}

fn request_has_browser_origin(headers: &HeaderMap) -> bool {
    headers.contains_key("origin")
}

fn unauthorized_upload_response() -> Response {
    (
        StatusCode::UNAUTHORIZED,
        Json(UploadResponse {
            success: false,
            result: None,
            message: Some("Missing or invalid PicNexus server token".to_string()),
        }),
    )
        .into_response()
}

pub async fn require_upload_auth(
    State(state): State<ServerRuntimeState>,
    request: Request<Body>,
    next: Next,
) -> Response {
    let should_reject = {
        if !request_has_browser_origin(request.headers()) {
            false
        } else {
            let headers = request.headers().clone();
            let query = request.uri().query().map(str::to_owned);
            let token_guard = state.auth_token.lock().await;
            let expected = token_guard.as_deref().unwrap_or("");
            !request_has_valid_server_token(&headers, query.as_deref(), expected)
        }
    };

    if should_reject {
        log::warn!("[Server] Rejected browser-origin upload without a valid token");
        return unauthorized_upload_response();
    }
...
pub async fn handle_upload(
    ...
    let Some(config) = config_guard.as_ref() else {
        return Json(UploadResponse {
            ...
            result: None,
            message: Some(
                "Server 未配置图床，请在「常规设置 → 编辑器兼容 Server」中选择默认图床".to_string(),
            ),
        });
    };
    ...
    for path in &file_paths {
        if let Err(e) = validate_image_file(std::path::Path::new(path)) {
            return Json(UploadResponse {
                success: false,
                result: None,
                message: Some(format!("图片校验失败 '{}': {}", path, e)),
            });
        }
...
pub async fn handle_file_upload(
    ...
    let detected_kind = match validate_image_bytes(&body, content_type) {
        Ok(kind) => kind,
        Err(message) => {
            return (
                StatusCode::OK,
    ...
    let filename = headers
        ...
        .map(|s| s.into_owned())
        .unwrap_or_else(|| {
            let ext = detected_kind.preferred_extension();
    ...
    let safe_ext = if detected_kind.matches_extension(requested_ext) {
        ...
        detected_kind.preferred_extension()
    };
...
async fn jd_get_aid() -> Result<(String, String), String> {
    ...
    let json: serde_json::Value =
        serde_json::from_str(json_str).map_err(|e| format!("JD aid JSON 解析失败: {}", e))?;
    ...
    if !status.is_success() {
        return match status {
            s if s == reqwest::StatusCode::UNAUTHORIZED => {
                Err("GitHub Token 无效或已过期".to_string())
            }
            s if s == reqwest::StatusCode::FORBIDDEN => {
                Err("GitHub API 频率限制，请稍后再试".to_string())
            }
...
async fn server_upload_smms(path: &std::path::Path, token: &str) -> Result<String, String> {
    ...
    if !status.is_success() {
        return match status {
            s if s == reqwest::StatusCode::UNAUTHORIZED => {
                Err("SM.MS Token 无效或已过期".to_string())
            }
...
async fn server_upload_imgur(path: &std::path::Path, client_id: &str) -> Result<String, String> {
    ...
    if !json["success"].as_bool().unwrap_or(false) {
        return Err(format!(
            "Imgur 上传失败: {}",
            json["data"]["error"].as_str().unwrap_or("未知错误")
        ));
    }
...
async fn server_upload_zhihu(
    ...
    let credentials_text = credentials_resp
        .text()
        .await
    ...
    if state != 1 {
        // 需要上传到 OSS
        let access_id = credentials["upload_token"]["access_id"]
            .as_str()
            .ok_or("知乎响应缺少 access_id")?
            .to_string();
        let access_key = credentials["upload_token"]["access_key"]
            .as_str()
            .ok_or("知乎响应缺少 access_key")?
            .to_string();
        let access_token = credentials["upload_token"]["access_token"]
            .as_str()
            .ok_or("知乎响应缺少 access_token")?
            .to_string();
        let object_key = credentials["upload_file"]["object_key"]
            .as_str()
            .ok_or("知乎响应缺少 object_key")?
            .to_string();
        ...
        let _ = client
            .put(format!(
                "https://api.zhihu.com/images/{}/uploading_status",
                image_id
            ))
            .header("Cookie", cookie)
            .header("Content-Type", "application/json")
    ...
    let zhihu_image_hash_re =
        regex::Regex::new(r"v2-[a-f0-9]+").map_err(|e| format!("创建知乎 URL 正则失败: {}", e))?;
    ...
    for _ in 0..30 {
        ...
        let status: serde_json::Value =
            serde_json::from_str(&status_text).map_err(|e| format!("解析状态响应失败: {}", e))?;
...
async fn server_upload_upyun(
    ...
    if !status.is_success() {
        return match status {
            s if s == reqwest::StatusCode::UNAUTHORIZED => {
                Err("又拍云认证失败: 请检查操作员账号和密码".to_string())
            }
            s if s == reqwest::StatusCode::NOT_FOUND => {
                Err(format!("又拍云服务空间不存在: {}", bucket))
            }
...
mod tests {
    ...
    #[test]
    fn request_has_browser_origin_only_when_origin_header_exists() {
        let mut headers = HeaderMap::new();

        assert!(!request_has_browser_origin(&headers));
    ...
    #[test]
    fn request_has_valid_server_token_accepts_supported_locations() {
        let mut headers = HeaderMap::new();
        headers.insert(SERVER_AUTH_TOKEN_HEADER, "secret".parse().unwrap());
        assert!(request_has_valid_server_token(&headers, None, "secret"));
        ...
        assert!(request_has_valid_server_token(
            &headers,
            Some("token=secret"),
            "secret",
        ));
    ...
    #[test]
    fn request_has_valid_server_token_rejects_missing_or_wrong_token() {
        let mut headers = HeaderMap::new();
        headers.insert(SERVER_AUTH_TOKEN_HEADER, "wrong".parse().unwrap());
    ...
    #[test]
    fn validate_image_bytes_rejects_fake_png_content() {
        let err = validate_image_bytes(b"not really a png", Some("image/png"))
            .expect_err("fake image should be rejected");

        assert!(err.contains("真实图片"));
    }

    #[test]
    fn validate_image_bytes_rejects_empty_body() {
        let err = validate_image_bytes(&[], Some("image/png"))
            .expect_err("empty body should be rejected");

        assert!(err.contains("请求体为空"));
    }

    #[test]
    fn validate_image_bytes_accepts_valid_small_png() {
        let kind =
            validate_image_bytes(&tiny_png(), Some("image/png")).expect("valid PNG should pass");

        assert_eq!(kind, DetectedImageKind::Png);
    }

    #[test]
    fn validate_image_bytes_accepts_svg_with_viewbox() {
        let svg = br#"<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 9"></svg>"#;
        let kind = validate_image_bytes(svg, Some("image/svg+xml"))
            .expect("SVG with dimensions should pass");

        assert_eq!(kind, DetectedImageKind::Svg);
    }

    #[test]
    fn validate_image_bytes_rejects_fake_ico_magic() {
        let err = validate_image_bytes(&[0x00, 0x00, 0x01, 0x00], Some("image/x-icon"))
            .expect_err("ICO header without directory entries should be rejected");

        assert!(err.contains("无法读取 ICO 尺寸"));
    }

    #[test]
    fn validate_image_bytes_rejects_oversized_body() {
        let bytes = vec![0_u8; MAX_SERVER_UPLOAD_SIZE + 1];
