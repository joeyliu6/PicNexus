...
- **历史表格的灯箱开关动画重做**：打开是一段连续的放大，不再"黑一下、停一拍、再放大"；关闭时大图精确落回
  你鼠标停着的那张预览卡片上，不再落偏或在最后一帧猛缩；背景幕布随动画淡出，不再被一把扯走。
  开场慢一点、收场快一点，两个方向用不同的缓动。收藏 / 时间轴视图的开关行为不变。
- 历史表格的缩略图现在有悬停反馈（描边 + 轻微提亮），此前鼠标停上去只有一个放大镜光标。
- 文档修复的确认框里现在会实时显示「这次会把哪些链接换成什么」的摘要，改策略（优先级 / 最快 / 手动）时摘要跟着变。
  此前确认框只让选策略，看不到会改什么。

### Fixed
- **备份同步不再用本地数据覆盖云端**：云端文件存在但内容为空或格式不对时，此前会被当成"云端没数据"，
  直接拿本地的覆盖上去并报"已同步"——另一台设备独有的记录就这么没了。现在这种情况会中止并说明原因。
- 从云端 / 本地文件导入历史时，两条路径此前用的是两套不同的判定标准，同一个文件走不同入口结果不一样；
  现在统一了，并且导入提示里会如实报出"跳过了几条无效记录"，不再虚报导入数。
- **修改备份密码后配置不再丢失**：托盘菜单是一个独立常驻的小窗口，它有自己的一份加密钥匙。此前主窗口换了钥匙
  它不知道，下次它写配置时用旧钥匙加密，主窗口下次启动就解不开、整份配置被重置成默认。现在换钥匙会通知它。
- 设置保存失败后不再卡在没保存成功的那份内容上：此前表单直接改写了内存里的缓存，写盘失败后"回滚"读到的
  还是那份被改过的缓存。
- 批量迁移取消（或连续失败自动中止）后，没轮到的条目不再永远停在"等待中"占着列表，统一归入"已跳过"，
  各项计数加起来等于总数。此前 300 条会有 200 条凭空消失。
- 批量迁移期间如果同一条记录被别处补了新镜像，结果不再重复写入同一个图床、虚报成功数。
- 灯箱里对单个镜像重新检测后，链接检测页不再把已经恢复的记录继续标成失效（此前要等下一轮批量检测才回正）。
- 文档修复扫描点了取消，此前已找到的备用链接仍会继续发网络请求去校验；现在取消就是全停。
- Cookie 登录：登录窗口的事件监控建不起来时，此前三层兜底一个都不生效——窗口开着、登录做完、然后永远没动静。
  现在会就地降级为轮询，并有看门狗兜底；超时提示也改成了指向真实存在的按钮（此前让你点一个不存在的按钮）。
- 图床 Cookie 测试、登录窗口打不开、"在浏览器打开"失败、文档修复里打开文件夹失败、CLI 加入 PATH 失败时，
  错误提示不再显示成 `[object Object]`，而是后端给的真实原因。macOS / Linux 上点登录授权会明确告诉你
  "当前系统暂不支持自动提取 Cookie"，此前这句话也被吞成了 `[object Object]`。
- 时间轴视图左侧的轨道线和年份分隔线现在能看见了——它们引用的 CSS 变量从没定义过，从写下那天起就没显示过。
  同一批还修了：所有弹出层（Popover）没有阴影、私有存储 profile 删除按钮没有边框、迁移筛选弹层字重不对、
  文档修复进度条在深色主题下没有渐变。已加静态门禁防止再犯。
