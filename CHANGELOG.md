...
### Changed
- **Windows 自动更新改走 NSIS 安装包**：下载体积从 65.7MB 降到 42.7MB，安装也不再走 Windows Installer 那套重流程。
  此前不管你当初用哪个安装包装的，自动更新一律拉 MSI——那条路要先卸旧版再装新版，实测慢一大截。
  MSI 仍然挂在 Release 页，供企业批量部署手动下载。
- Windows 上更新下载完成后会显示「正在安装更新」，并说明应用会自动关闭、装完自行重启。
  此前进度条一走完窗口就凭空消失，中间一分多钟屏幕上什么都没有，很容易以为卡死了。
  （macOS / Linux 行为不变，仍是停在「重启完成更新」等你点。）
- 更新失败时显示中文原因和该怎么办，英文技术原文移进副行的悬停提示里保留给排障，
  不再把一长串报错 URL 折成三行；同时去掉了与卡片内容重复的那条 toast。

### Added
- Obsidian 插件的设置项现在能被 Obsidian 1.13 的设置搜索检索到（改用声明式设置 API，仍兼容 1.4.0）。

### Fixed
- 历史记录删除后显示的是**实际删掉的条数**，不再虚报：已经在别处删过的记录不会被重复计数，
  多窗口下也不会各减一次；批量删除时单条失败也不再牵连其余记录。
- 删除 / 清空历史后，按时间维度的统计会立即刷新。此前它会一直停在旧数字，直到下一次历史更新事件才回正。
- 统一了镜像「可用」的判据——必须状态成功**且**链接非空。此前只看状态，
  一条没有链接的「成功」会让记录在数据库侧存活、界面侧却已删除，批量迁移的候选数量也会虚高。
- 浅色主题下打开登录窗口不再先闪一下深色背景（首屏骨架此前硬编码了深色底）。
- 上传队列的缩略图右下角不再各被削掉 1px。
- 修改主题 / 压缩 / 快捷键等配置不再触发图床按钮状态的全量重算和随之而来的磁盘写入。
  此前配置更新是全量广播，改任何一项都会让这块从头算一遍。
- Obsidian 插件状态栏在切回 Obsidian 窗口时立刻刷新，点一下也能强制刷新。
  此前只有 30 秒一次的轮询，在桌面端换了图床最多要等半分钟才看得到。
- Obsidian 插件端口填超范围时会给出提示，不再静默丢弃；设置也不再每敲一个键就往磁盘写一次。
- Obsidian 插件「上传通知」开关的说明改得与实际行为一致——原文声称管失败通知，这个开关其实管不到。
...
**受影响的人**：所有已经配置过又拍云的用户。升级后又拍云会显示为「未配置」。

**要做什么**：去又拍云控制台「操作员授权 → S3 访问凭证」单独生成一对凭证，
填进设置页又拍云卡片新增的「S3 AccessKey / S3 SecretKey」两格。

**为什么**：又拍云的 S3 兼容端点不认操作员账号密码，拿它签名会被回
`ErrInvalidAccessKeyID`。旧版本正是这么干的，所以**软件内上传这条路以前从未成功过**
（Typora / Obsidian / CLI 走的是另一条 REST 链路，一直是好的，不受影响）。
补上新凭证后软件内上传才第一次真正可用。

> 旧版本的「测试连接」只验 REST 链路，会在上传其实不可用时亮绿灯。
> 本版起测试连接会 REST + S3 两条各验一次，并在报错里点明是哪一对凭证有问题。

### Added
- 新增 **WebDAV 私有存储图床**：可以把 NAS / 自建服务器当图床用，支持多实例配置，
  Typora / Obsidian / CLI 编辑器链路同样可用，配置加密保存；
  连接测试能区分 403 / 密码错误 / 仅支持 Digest 的服务器。
- 已保存的密码 / Token / Cookie / 密钥现在**可以查看和原地编辑了**（按需显示明文），也**可以删掉了**：把输入框清空再点别处，确认后即删除。
  此前这些字段只能覆盖、无法清除（内置图床不像 WebDAV 能整个删掉配置），
  框里那句「留空则不修改」还会在你刚删完时告诉你这下不算。
  确认框里可勾「本次运行内不再提示」，方便一口气清理多个；重启后恢复提示。
- 又拍云配置新增「S3 AccessKey / S3 SecretKey」，与操作员账号密码分开；卡片内有说明去哪里获取。
- 公开访问域名支持明文 HTTP：填 `http://` 域名时会先请求确认一次，确认按主机名生效，换域名自动失效。
  又拍云免费测试域名只有 HTTP，此前这类图床整个不可用。
- R2 缩略图代理（wsrv.nl）改为可关闭，并在代理不可达时自动回落到原图。

### Fixed
- 又拍云软件内上传改用 S3 凭证签名，这条链路首次真正可用。
- 又拍云编辑器链路现在遵守「存储路径」配置，并对同名文件做唯一化，不再互相覆盖。
- 所有 S3 上传路径（含编辑器 / CLI / 本地 Server）都会带上真实的 Content-Type，
  浏览器打开图片链接显图而不是弹下载框。
- WebDAV 局域网图床（NAS / 自建）的缩略图不再被 CSP 拦掉——此前一直是坏的。
- 在历史页收藏图片后，收藏页立刻能看到，不用再刷新一次。
- S3 图床的错误提示不再只说一句没有信息量的 `service error`，会指出是密钥不对还是存储桶不存在。
- 缩略图候选链在链路更新时不再误报失败、不再卡住骨架屏。
- 链接检测：3xx 跳转不再误判为死链；被安全策略拦截的链接会标明原因而不是报网络错误；
  删除历史记录后检测列表不再残留幽灵行；代理 fake-ip 环境下不再整批误判。
- MD 修复工具：拒绝非 UTF-8 文件避免写回乱码；备份按文件隔离，扫描时跳过备份目录。
- 批量迁移的结果落盘后立即广播刷新，不再因页面缓存丢失。
- 链接检测的 Content-Type 判断在三个调用点上统一。
- 设置页不再提示公开域名必须是 HTTPS；WebDAV 覆盖开关不再被挤变形。

### Security
- 灯箱大图、表格悬停预览、灯箱 ±1 预热接入图片地址安全闸，与缩略图同判据。
  CSP 的 `img-src` 为支持 HTTP 图床放开了 `http:`，这三条路此前仅靠 CSP 兜底。
- 收藏页与时间线的缩略图接入同一道闸。
- 链路本地地址（含云元数据 `169.254.169.254`）在任何放行策略下都拒绝，
  即使用户显式确认过该主机名。
...
### Changed
- 云端配置上传和双向同步现在必须设置备份密码；本地明文导出仍保留风险确认流程。
- 历史列表、搜索、收藏和流式读取统一使用稳定的时间戳与 ID 排序；多图床上传结束后统一历史、缩略图和剪贴板的主图床。

### Fixed
- 修复同时间戳记录跨页可能重复或遗漏、后台更新覆盖收藏状态、重试路径回退到另一条历史记录等一致性问题。
- 修复 SM.MS 重复图片、纳米 STS 连接测试误报成功、压缩预览参数回归及 WebP 超尺寸/图片头解析失败的错误处理。
- 修复编辑器 Server `/status` 可被跨域读取的问题，同时保持原生客户端连接检测兼容。

### Security
- 上传读取在分配缓冲区前执行大小检查；未声明服务上限的上传统一限制为 50MB。
- 纳米 token sidecar 改用 stdin JSON 传递凭据，避免 Cookie 和 Auth-Token 出现在进程参数中。
