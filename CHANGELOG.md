...
每个版本一节，一行一条、面向用户、按感知度排序。发版时 CI 会把对应版本的段落原样贴到
GitHub Release 正文（`scripts/extract-changelog-section.mjs`），所以这里写的就是 Release 说明本身：
只写用户能感知的变化，不写重构 / 测试 / CI 这类内部改动，不写「此前……现在……」的长段叙述。
1.0.x 各节是旧格式，保留不动。
...
- 历史表格的灯箱开关动画重做：打开是一段连续的放大，关闭时精确落回鼠标所在的预览卡片，背景随动画淡出；打开动画进行中触发关闭不再卡住后续打开；缩略图新增悬停反馈。
- 文档修复的确认框实时显示「哪些链接会换成什么」的摘要，切换策略时同步更新。
- 修复备份同步用本地数据覆盖云端的问题：云端文件为空或格式不对时会中止并说明原因，不再当成「云端没数据」。
- 修复修改备份密码后整份配置被重置的问题：托盘窗口现在会同步拿到新钥匙，不再用旧钥匙写坏配置。
- 修复设置保存失败后「回滚」仍停在改过的内容上的问题。
- 备份「同步」现在如实反馈：两边都没变化时提示「已是最新」并跳过上传，有变化时分别报出上传 / 下载的条数，不再永远显示全表条数。
- 从云端 / 本地文件导入历史时两条路径的判定统一，导入提示如实报出跳过的无效记录数，不再虚报。
- 批量迁移取消或自动中止后，未处理条目归入「已跳过」而不是永远停在「等待中」，各项计数之和等于总数；同一记录被别处补了镜像时不再重复写入、虚报成功。
- 灯箱里对单个镜像重新检测后，链接检测页立即摘掉已恢复记录的失效标记，不用等下一轮批量检测。
- 文档修复扫描取消后不再继续发网络请求校验备用链接。
- Cookie 登录窗口的事件监控建不起来时自动降级为轮询并有看门狗兜底，不再「登录做完了却永远没动静」；超时提示改为指向真实存在的按钮。
- Cookie 测试、登录窗口、「在浏览器打开」、打开文件夹、CLI 加入 PATH、WebDAV 备份同步、图片压缩预览等操作失败时显示真实原因，不再是 `[object Object]`；macOS / Linux 上会明确提示暂不支持自动提取 Cookie。
- 修复时间轴视图轨道线和年份分隔线从未显示的问题，同批修复弹出层无阴影、私有存储删除按钮无边框、迁移筛选弹层字重不对、深色主题下文档修复进度条无渐变；已加静态门禁防止再犯。

---

## [1.1.1] - 2026-08-24

- Windows 自动更新改用 NSIS 安装包，下载体积由 65.7 MB 降到 42.7 MB，安装不再走 Windows Installer 的重流程（本次更新自身仍需清理旧的 MSI 安装，提速从下个版本起生效）。
- Windows 安装期间会提示「正在安装更新」并说明应用将自动关闭重启，不再出现窗口消失后长时间没有任何反馈的空窗。
- 更新失败时显示中文原因和处理建议，英文原文移入悬停提示保留给排障，不再重复弹出同内容的 toast。
- 修复历史记录删除虚报条数：单条与批量删除均按实际结果计数，批量删除中单条失败不再影响其余记录。
- 修复删除 / 清空历史后时间维度统计不刷新的问题。
- 统一镜像「可用」判据为状态成功且链接非空，修复删除后数据库与界面不一致、批量迁移候选数量虚高的问题。
- 修复浅色主题下登录窗口首屏闪深色背景、上传队列缩略图右下各被削 1px 的问题。
- 修改主题 / 压缩 / 快捷键等配置不再触发图床按钮状态的全量重算。
- Obsidian 插件更新至 1.0.3：设置项可被 Obsidian 1.13 的设置搜索检索，状态栏在切回窗口时立即刷新（原为 30 秒轮询），端口越界会给出提示且不再每次按键写盘。

---

## [1.1.0] - 2026-08-21

- 新增 WebDAV 私有存储图床，可把 NAS / 自建服务器当图床用，编辑器和 CLI 链路同样支持。
- ⚠️ 又拍云需补填「S3 AccessKey / S3 SecretKey」（控制台「操作员授权 → S3 访问凭证」生成），否则会显示为未配置。
- 公开访问域名支持明文 HTTP，确认一次后生效，仅有 HTTP 域名的图床从此可用。
- 已保存的密码 / Token / Cookie / 密钥支持查看、原地编辑和删除。
- S3 上传统一携带真实 Content-Type 并对同名文件去重，图片链接不再弹下载框、不再互相覆盖。
- 修复链接检测 3xx 误判死链、删除记录后残留幽灵行等问题。
- 修复 MD 修复工具乱码写回、批量迁移结果丢失、收藏页不即时刷新的问题。
- 改进 WebDAV 备份的错误提示，备份密码加密存储且不再被意外清空。
- 加强图片地址安全检查，缩略图、灯箱和悬停预览统一走安全闸。
- 优化历史页大列表加载和上传内存占用。
...
> 2026-01-23 至 2026-03-23 的功能批次，随 v1.0.3 发布。
